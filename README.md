...
当前仓库根目录包含 53 个可导入应用。下表根据各应用的 `config.json` 整理，方便用户快速了解应用名称、类型和能力范围，而不需要逐个打开目录确认。

其中 `X` 系列和腾讯云安全模拟系列不依赖外部安全产品、网络设备、IT 系统或 SaaS 服务，适合开箱体验内置剧本、演示编排流程和做本地验证。
...
以下 app 仅模拟腾讯云/TCE 安全产品常见的输入、输出、策略状态和审计行为，不连接腾讯云，也不需要 SecretId、SecretKey。贯通样例和三套 Demo 剧本蓝图见[腾讯云安全产品模拟器与 Demo 剧本设计](docs/tencent-cloud-security-simulator-plan.md)。

| 应用 | 分类 | 动作数 | 说明 |
| --- | --- | ---: | --- |
| [腾讯云 SOC（模拟）](shakespeare-action-python-tencent_soc_sim/) | 安全产品 | 8 | 告警汇聚、事件分级、Syslog/Kafka 链路健康和处置回写 |
| [腾讯云资产中心（模拟）](shakespeare-action-python-tencent_asset_sim/) | 云服务 | 11 | 主机、业务系统、负责人、VPC 和互联网暴露面上下文 |
| [腾讯天幕 NIPS（模拟）](shakespeare-action-python-tencent_tianmu_sim/) | 安全产品 | 6 | 旁路 IP 阻断、解封、策略查询、TTL 到期和审计 |
| [腾讯云防火墙 CFW（模拟）](shakespeare-action-python-tencent_cfw_sim/) | 安全产品 | 7 | VPC 间访问控制、策略同步、TTL 回滚和审计 |
| [腾讯云 WAF（模拟）](shakespeare-action-python-tencent_waf_sim/) | 安全产品 | 8 | 域名防护、攻击事件、IP 黑名单、解封和 TTL 审计 |
| [腾讯云主机安全 HIDS/CWP（模拟）](shakespeare-action-python-tencent_hids_sim/) | 安全产品 | 7 | 主机告警、主机信息、进程和外联线索 |
| [腾讯云容器安全 TCSS（模拟）](shakespeare-action-python-tencent_tcss_sim/) | 安全产品 | 7 | 集群、运行时告警、异常进程和恶意外联线索 |
