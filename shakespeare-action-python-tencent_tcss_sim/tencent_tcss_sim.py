...
APP_NAME = 'tencent_tcss_sim'
PRODUCT_NAME = '腾讯云容器安全 TCSS（模拟）'
APP_DESCRIPTION = '离线模拟腾讯云容器安全的集群、运行时告警、异常进程、恶意外联和健康状态'
APP_VERSION = '1.0.0'
DEFAULT_RECORDS = json.loads('[{"id": "cls-prod-checkout", "kind": "tcss_cluster", "name": "prod-checkout-tke", "ip": "10.30.0.10", "owner": "platform-team", "status": "protected", "severity": "medium", "summary": "统一收银台生产 TKE 集群，运行时防护已启用", "data": {"cluster_id": "cls-prod-checkout", "asset_id": "asset-tke-prod-checkout", "business_id": "biz-checkout", "region": "ap-guangzhou", "node_count": 6, "sensor_online": 6}}, {"id": "tcss-alert-20260724-0001", "kind": "tcss_alert", "name": "容器异常外联", "ip": "198.51.100.66", "owner": "platform-team", "status": "new", "severity": "high", "summary": "checkout-api Pod 连接已知矿池地址", "data": {"alert_id": "tcss-alert-20260724-0001", "cluster_id": "cls-prod-checkout", "namespace": "checkout-prod", "workload": "checkout-api", "pod_name": "checkout-api-7cdd7d9c8b-h2k9s", "node_ip": "10.30.1.21", "src_ip": "10.30.2.17", "dst_ip": "198.51.100.66", "external_ip": "198.51.100.66", "dst_port": 3333, "process_name": "xmrig", "alert_type": "malicious_outbound", "confidence": 97, "trace_id": "demo-trace-container-0005", "occurred_at": "2026-07-24T10:28:19+08:00"}}, {"id": "tcss-process-20260724-0001", "kind": "tcss_process", "name": "xmrig", "ip": "10.30.2.17", "owner": "platform-team", "status": "running", "severity": "high", "summary": "容器中发现矿池进程 xmrig", "data": {"cluster_id": "cls-prod-checkout", "namespace": "checkout-prod", "pod_name": "checkout-api-7cdd7d9c8b-h2k9s", "pid": 884, "command_line": "/tmp/xmrig -o 198.51.100.66:3333", "trace_id": "demo-trace-container-0005"}}]')
ACTION_DEFINITIONS = json.loads('[{"action": "health_check", "description": "检查模拟 API、鉴权和组件状态", "operation": "health", "classify": "query", "parameters": {}, "sample": {}, "record_kinds": [], "filter_map": {}, "target_params": []}, {"action": "query_clusters", "description": "查询容器集群与传感器状态", "operation": "query", "classify": "query", "parameters": {"cluster_id": {"data_type": "string", "description": "TKE 集群 ID", "required": false, "order": 0}, "status": {"data_type": "string", "description": "防护状态", "required": false, "order": 1}}, "sample": {"cluster_id": "cls-prod-checkout"}, "record_kinds": ["tcss_cluster"], "filter_map": {"cluster_id": ["id", "cluster_id"], "status": ["status"]}, "target_params": []}, {"action": "query_runtime_alerts", "description": "查询容器运行时安全告警", "operation": "query", "classify": "query", "parameters": {"alert_id": {"data_type": "string", "description": "容器安全告警 ID", "required": false, "order": 0}, "cluster_id": {"data_type": "string", "description": "TKE 集群 ID", "required": false, "order": 1}, "severity": {"data_type": "string", "description": "告警等级", "required": false, "order": 2}, "status": {"data_type": "string", "description": "告警状态", "required": false, "order": 3}, "limit": {"data_type": "integer", "description": "返回数量", "required": false, "order": 4, "default_value": 50}}, "sample": {"cluster_id": "cls-prod-checkout", "severity": "high"}, "record_kinds": ["tcss_alert"], "filter_map": {"alert_id": ["id", "alert_id"], "cluster_id": ["cluster_id"], "severity": ["severity"], "status": ["status"]}, "target_params": []}, {"action": "query_runtime_processes", "description": "查询容器异常进程线索", "operation": "query", "classify": "query", "parameters": {"cluster_id": {"data_type": "string", "description": "TKE 集群 ID", "required": true, "order": 0}, "pod_name": {"data_type": "string", "description": "Pod 名称", "required": false, "order": 1}, "process_name": {"data_type": "string", "description": "进程名", "required": false, "order": 2}, "limit": {"data_type": "integer", "description": "返回数量", "required": false, "order": 3, "default_value": 50}}, "sample": {"cluster_id": "cls-prod-checkout", "process_name": "xmrig"}, "record_kinds": ["tcss_process"], "filter_map": {"cluster_id": ["cluster_id"], "pod_name": ["pod_name"], "process_name": ["name"]}, "target_params": []}, {"action": "query_malicious_connections", "description": "查询容器恶意外联线索", "operation": "query", "classify": "query", "parameters": {"cluster_id": {"data_type": "string", "description": "TKE 集群 ID", "required": true, "order": 0}, "external_ip": {"data_type": "string", "description": "外联 IP", "required": false, "order": 1}, "workload": {"data_type": "string", "description": "工作负载名称", "required": false, "order": 2}, "limit": {"data_type": "integer", "description": "返回数量", "required": false, "order": 3, "default_value": 50}}, "sample": {"cluster_id": "cls-prod-checkout", "external_ip": "198.51.100.66"}, "record_kinds": ["tcss_alert"], "filter_map": {"cluster_id": ["cluster_id"], "external_ip": ["external_ip", "dst_ip", "ip"], "workload": ["workload"]}, "target_params": []}, {"action": "update_alert_status", "description": "更新容器安全告警状态", "operation": "update_record", "classify": "write", "parameters": {"alert_id": {"data_type": "string", "description": "容器安全告警 ID", "required": true, "order": 0}, "status": {"data_type": "string", "description": "目标状态", "required": false, "order": 1, "default_value": "processing"}, "comment": {"data_type": "string", "description": "处置说明", "required": false, "order": 2, "default_value": "SOAR 已联动网络侧阻断"}, "trace_id": {"data_type": "string", "description": "SOAR 链路追踪 ID", "required": false, "order": 10, "default_value": "demo-trace-web-0001"}, "operator": {"data_type": "string", "description": "操作者账号", "required": false, "order": 11, "default_value": "soar-demo"}, "trigger_source": {"data_type": "string", "description": "触发来源，例如 soc_alert、manual、scheduled", "required": false, "order": 12, "default_value": "soc_alert"}, "approval_id": {"data_type": "string", "description": "审批单 ID；需要审批时由剧本传入", "required": false, "order": 13}}, "sample": {"alert_id": "tcss-alert-20260724-0001", "status": "processing", "comment": "已阻断矿池外联 IP", "trace_id": "demo-trace-container-0005"}, "record_kinds": ["tcss_alert"], "filter_map": {"alert_id": ["id", "alert_id"]}, "target_params": ["alert_id"]}, {"action": "query_operation_history", "description": "查询产品模拟操作审计记录", "operation": "history", "classify": "query", "parameters": {"trace_id": {"data_type": "string", "description": "按 trace_id 过滤", "required": false, "order": 0}, "target": {"data_type": "string", "description": "按目标 IP、资产或策略 ID 过滤", "required": false, "order": 1}, "limit": {"data_type": "integer", "description": "返回数量", "required": false, "order": 2, "default_value": 50}}, "sample": {"trace_id": "demo-trace-web-0001", "limit": 20}, "record_kinds": [], "filter_map": {}, "target_params": []}]')
HEALTH_COMPONENTS = json.loads('["容器安全云 API", "运行时传感器", "K8s API 审计", "告警检索接口"]')
...
def query_clusters(params, assets, context_info):
    """查询容器集群与传感器状态"""
    return _execute('query_clusters', params, assets, context_info)


def query_runtime_alerts(params, assets, context_info):
    """查询容器运行时安全告警"""
    return _execute('query_runtime_alerts', params, assets, context_info)


def query_runtime_processes(params, assets, context_info):
    """查询容器异常进程线索"""
    return _execute('query_runtime_processes', params, assets, context_info)


def query_malicious_connections(params, assets, context_info):
    """查询容器恶意外联线索"""
    return _execute('query_malicious_connections', params, assets, context_info)
