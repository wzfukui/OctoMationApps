...
## 主要功能
  ...
  - ✅活动列表查存：支持按key、value、key+value判断记录是否存在，可选时间窗口
...
- 使用活动列表之前，需要先初始化该列表
- 活动列表名称必须是没有空格的字符串，且只能包含字母、数字、下划线、中划线。MySQL表名最长64字符，本APP自动增加`_al_`前缀，因此活动列表名称最长60字符
...
- 建表会创建单字段索引和组合索引：`_key`、`_value(255)`、`create_time`、`update_time`、`(_key, create_time)`、`(_value(255), create_time)`、`(_key, _value(255), create_time)`
...
- 2026-07-04 v1.1.5 恢复并扩展查存能力，支持按key、value、key+value判断；新增组合索引并修复查询SQL参数化
